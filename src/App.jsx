  ...
  return (
    ...
    <RecentSearch recentHistory={recentHistory} setRecentHistory={setRecentHistory} setSelectedHistory={setSelectedHistory}/>
      ...
      <h1 className='text-4xl bg-clip-text text-transparent bg-gradient-to-r from-pink-700 to-violet-700'>
        Here to help - ask me anything!!</h1>
        {
          loader?
              <div role="status">
    ...
    <span className="sr-only">Loading...</span>
            ...
            <ul>
            {
              result.map((item,index)=>(
                <div key={index+Math.random()} className={item.type=='q'?'flex justify-end':''}>
                  {
                item.type=='q'?
                <li key={index+Math.random()} className='text-right p-1 border-8 dark:bg-zinc-700 bg-pink-50 dark:border-zinc-700 border-pink-50 rounded-tl-3xl rounded-br-3xl rounded-bl-3xl w-fit'><Answer ans={item.text} totalResult={1} index={index} type={item.type} /></li>
        ...
        <div className='dark:bg-zinc-800 bg-pink-100 w-1/2 p-1 pr-5 dark:text-white text-zinc-800 m-auto rounded-4xl
